...
export function createSystemCalls(
  ...
  const moveTo = async (inputX: number, inputY: number) => {
    ...
    try {
      ...
      const tx = await worldSend("F", [[x, y,0,0,1,0]]);
  ...
  const putMarker = async () => {
    ...
    const [x, y] = wrapPosition(playerPosition.x, playerPosition.y);

    // const tx = await worldSend("putMarker", []);
    const tx = await worldSend("F", [[x,y,0,1,0,0]]);
    await awaitStreamValue(txReduced$, (txHash) => txHash === tx.hash);
  };

  const reset = async () => {
    const tx = await worldSend("F", [[0,0,0,0,0,1]]);
    await awaitStreamValue(txReduced$, (txHash) => txHash === tx.hash);
  }
